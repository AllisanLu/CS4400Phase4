...
begin
	...
	update bank_account join savings on bank_account.accountID = savings.accountID and bank_account.bankID = savings.bankID
    	...
    	set resAssets =
        	case
            	when ifnull(balance, 0) >= ifnull(minBalance, 0) then ifnull(resAssets, 0) - ifnull(truncate((interest_rate/100) * balance, 0), 0)
            	else ifnull(resAssets, 0)
        	end;
	...
	update bank_account join market on bank_account.accountID = market.accountID and bank_account.bankID = market.bankID
    	...
    	set resAssets =
        	case
            	when ifnull(numWithdrawals, 0) <= ifnull(maxWithdrawals, 0) then ifnull(resAssets, 0) - ifnull(truncate((interest_rate/100) * balance, 0), 0)
            	else ifnull(resAssets, 0)
        	end;
	...
	update bank_account join savings on bank_account.accountID = savings.accountID and bank_account.bankID = savings.bankID
    	...
    	set balance =
        	case
            	when ifnull(balance, 0) >= ifnull(minBalance, 0) then ifnull(balance, 0) + ifnull(truncate((interest_rate/100) * balance, 0), 0)
            	else ifnull(balance, 0)
        	end;
	...
	update bank_account join market on bank_account.accountID = market.accountID and bank_account.bankID = market.bankID
    	...
    	set balance =
        	case
            	when ifnull(numWithdrawals, 0) <= ifnull(maxWithdrawals, 0) then ifnull(balance, 0) + ifnull(truncate((interest_rate/100) * balance, 0), 0)
            	else ifnull(balance, 0)
        	end;
end //
delimiter ;
