...
-- Bank Management Project - Phase 3 (v3)
...
begin
    if (ip_corpID in (select corpID from corporation) 
		and ip_bank_employee in (select perID from employee) and ip_bank_employee not in (select manager from bank)
        and ip_manager in (select perID from employee) and ip_manager not in (select perID from workfor) and ip_manager not in (select manager from bank)
        and ip_bankID not in (select bankID from bank)) then
...
begin
    ...
    and ip_perID not in (select manager from bank)) then
        if (ip_salary >=0) then
...
begin
    ...
    if (ip_perID in (select perID from employee) and ip_perID not in (select manager from bank) and ip_perID not in (select perID from workfor)) then
		if (ip_salary >=0) then
			update bank set manager = ip_perID where ip_bankID = bankID;
			update employee set salary = ip_salary where perID = ip_perID;
		end if;
	end if;
end //
...
begin
    if (ip_requester in (select perID from access where bankID = ip_bankID and accountID = ip_accountID) and ip_deposit_amount >= 0) then
		update bank_account set balance = coalesce(balance, 0) + ip_deposit_amount where bankID = ip_bankID and accountID = ip_accountID;
...
begin
    -- If the person making the withdrawal does not have access to the account then don't change the database state
    if (ip_requester in (select perID from access where bankID = ip_bankID and accountID = ip_accountID) and ip_withdrawal_amount >= 0) then
		if exists (select * from bank_account where bankID = ip_bankID and accountID = ip_accountID and coalesce(balance, 0)-ip_withdrawal_amount >=0) then
        ...
        elseif exists (select protectionBank from checking where ip_bankID = bankID and ip_accountID = accountID) then
			if exists (select * from bank_account
                ...
                and coalesce(bank_account.balance, 0) - ip_withdrawal_amount + coalesce(overdraft_acct.balance, 0) > 0) then
...
begin
    ...
    and ip_transfer_amount >= 0) then
        if exists (select * from bank_account where bankID = ip_from_bankID and accountID = ip_from_accountID and coalesce(balance, 0)-ip_transfer_amount >=0) then
			update bank_account set balance = balance - ip_transfer_amount where bankID = ip_from_bankID and accountID = ip_from_accountID;
			update bank_account set balance = coalesce(balance, 0) + ip_transfer_amount where bankID = ip_to_bankID and accountID = ip_to_accountID;
			...
			update access set dtAction = ip_dtAction where perID = ip_requester and bankID = ip_to_accountID and accountID = ip_to_accountID;
            
            if exists (select * from market where ip_bankID = ip_from_bankID and ip_accountID = ip_from_accountID) then
				update market set numWithdrawals = numWithdrawals + 1 where ip_bankID = ip_from_bankID and ip_accountID = ip_from_accountID;
			end if;
            
            if ((ip_to_bankID, ip_to_accountID) in (select bankID, accountID from interest_bearing)) then
				update interest_bearing set dtDeposit = ip_dtAction where bankID = ip_to_bankID and accountID = ip_to_accountID;
			end if;
        
        elseif exists (select protectionBank from checking where bankID = ip_from_bankID and accountID = ip_from_accountID) then
			if exists (select * from bank_account
                join checking on bank_account.bankID = checking.bankID and bank_account.accountID = checking.accountID
                join bank_account as overdraft_acct on checking.protectionBank = overdraft_acct.bankID and checking.protectionAccount = overdraft_acct.accountID
                where ip_from_bankID = bank_account.bankID and ip_from_accountID = bank_account.accountID
                and coalesce(bank_account.balance, 0) - ip_transfer_amount + coalesce(overdraft_acct.balance, 0) > 0) then
				
                -- Update protection account balance
                update bank_account join checking on bank_account.bankID = checking.bankID and bank_account.accountID = checking.accountID
					join bank_account as overdraft_acct on checking.protectionBank = overdraft_acct.bankID and checking.protectionAccount = overdraft_acct.accountID
					set overdraft_acct.balance = coalesce(overdraft_acct.balance, 0) - (ip_transfer_amount - coalesce(bank_account.balance, 0))
					where ip_from_bankID = bank_account.bankID and ip_from_accountID = bank_account.accountID;
